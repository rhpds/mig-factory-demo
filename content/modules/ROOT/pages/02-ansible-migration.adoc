...
. On the left menu, click on *Automation Execution* -> *Templates*, and then click on *01 Migrate VMware VMs to OpenShift Virtualization*.
...
WARNING: We need to choose a VM that has not been migrated yet. In the first module, we manually migrated the database-{openshift_cluster_user_name} and winweb01-{openshift_cluster_user_name} machines. So here, we will migrate the remaining VM: *winweb02-{openshift_cluster_user_name}*.
...
[subs="+attributes"]
----
winweb02-{openshift_cluster_user_name}
----
...
. Navigate to the link:{ocp_console_url}++/k8s/ns/openshift-mtv/forklift.konveyor.io~v1beta1~Plan++[OpenShift Console^]
+
...
Ansible Automation Platform has started the VM migration plan, and the *winweb02-{openshift_cluster_user_name}* VM migration commences.
