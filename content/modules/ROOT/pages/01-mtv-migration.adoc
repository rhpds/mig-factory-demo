...
. Click *winweb01-user1* in the list of virtual machines.
+
...
In all likelyhood, we have removed the permissions to power on these VMs.
...
. You can find the VMs quickly by using the *name:* filter, and providing your username: `user1`
...
+
NOTE: Storage and network mappings should be automatically detected from the discovered virtual machines, but make sure to double-check that the correct values are set as described here.
...
image::module-01/NN_Create_Migration_Plan_3_cnv.png[link=self, window=blank, width=80%]
endif::[]
ifeval::["{ocp_cloud}" == "rosa"]
image::module-01/NN_Create_Migration_Plan_3_rosa.png[link=self, window=blank, width=80%]
endif::[]
*SAY:*
...
+
NOTE: OpenShift Route is providing load balancing between the *winweb01-user1* and *winweb02-user1* hosts, so you might see either hostname reported.
+
