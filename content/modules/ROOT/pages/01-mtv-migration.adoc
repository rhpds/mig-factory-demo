...
* `database-{openshift_cluster_user_name}`
* `winweb01-{openshift_cluster_user_name}`
...
Click the *Workloads* icon and expand the navigation tree until you see the folder *Roadshow* and the four VMs under it.
...
. Click `winweb01-{openshift_cluster_user_name}` in the list of virtual machines.
+
...
. You can find the VMs quickly by using the *name:* filter, and providing your username: `{openshift_cluster_user_name}`
...
+
NOTE: Storage and network mappings should be automatically detected from the discovered virtual machines, but make sure to double-check that the correct values are set as described in this openshift_cluster_user_namee.
...
. The main delay is the validation of the VDDK init image.
Scroll down to the bottom of the page to show the *Conditions* and progress of plan preparation, and show that the validation of the VDDK init image is in progress.
image::module-01/NN_validating_vddk_init_image.png[link=self, window=blank, width=50%]
...
+
. After a few moments the plan will become *Ready*.
Begin the migration by clicking *Start Migration*.
+
...
+
. Click on the `winweb01-{openshift_cluster_user_name}` VM that is running and click on its name to see the VM details page.
+
...
+
NOTE: OpenShift Route is providing load balancing between the *winweb01-{openshift_cluster_user_name}* and *winweb02-{openshift_cluster_user_name}* hosts, so you might see either hostname reported.
+
