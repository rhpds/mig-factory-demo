...
.. From the top bar, select *All Clusters*
.. On the left, click *Home* and then *Search*.
...
. Let's navigate deeper. Click on the *winweb01-user1*.
+
...
+
. Navigate to the Related Resources tab to showcase all of the resources that are associated with the *winweb01-user1* virtual machine, this allows for easy troubleshooting when issues come up.
+
...
.. You may have to log in, using the OpenShift cluster-admin credentials, *user:* `{ocp_admin_username}` *password:* `{ocp_admin_password}`
+
...
+
ifeval::["{ocp_cloud}" == "cnv"]
. You will see data in the *Top 50 Max Latency API Server* title followed by *View*.
Click on *View*.
+
...
. Look at the workloads inside our cluster like Virtual Machines.
Navigate to the side bar and click on *Browse*.
+
...
+
. Now look at optimizing the resources used by our Virtual Machines.
Click back to the *Browse* dashboard and select *ACM - OpenShift Virtualization Resource Optimization / Cluster*
+
image::module-03/ocp-acm-grafana-resource-optimization.png[link=self, window=blank, width=80%]
+
. This dashboard provides a detailed view of the resource usage and optimization for the cluster.
You can select a specific namespace to view the resource usage and optimization for that namespace.
Scroll down to find he *openshift-mtv* namespace that received the migrating virtual machines, and talk about watching the resource usage and optimization for the namespace as virtual machines migrate.
+
image::module-03/ocp-acm-grafana-resource-optimization-namespace.png[link=self, window=blank, width=80%]
+
